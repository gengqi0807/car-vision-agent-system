...
2. `local-model`：直接使用新合并分支中的本地交警手势模型进行视频识别。
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="手势识别调试入口")
    parser.add_argument(
        "--mode",
...
def run_local_model(args):
    ...
    frame_delay = 1.0 / (fps_video * 0.6) if fps_video > 0 else 1.0 / 15
    print(
    ...
    while True:
        ...
        if last_landmarks is None:
            frame = draw_chinese_text(frame, "未检测到人体", (10, status_y), (0, 0, 255), 36)
        elif state_machine.state == police_cfg.STATE_ACTIVE:
            frame = draw_chinese_text(frame, "动作识别中...", (10, status_y), (0, 255, 255), 30)
            ...
            frame = draw_chinese_text(frame, f"交警手势: {display_result}", (10, status_y), (0, 255, 0), 36)
            if display_confidence > 0:
                frame = draw_chinese_text(frame, f"置信度: {display_confidence:.0%}", (10, 75), (0, 200, 0), 22)
        else:
            frame = draw_chinese_text(frame, "等待动作...", (10, status_y), (255, 255, 255), 30)
        ...
        if dl_engine is not None:
            ...
            frame = draw_chinese_text(frame, dl_gesture, (panel_x + 10, panel_y + 32), (0, 215, 255), 28)
            cv2.putText(
                frame,
                f"conf: {dl_confidence:.1%}",
