...
const yellowPlateVehicleTypes = new Set(["卡车", "公交", "吊车"]);

function normalizePlateColorForDisplay(plateNumberRaw: string, plateColor: string, vehicleType: string) {
  const plateNumber = (plateNumberRaw || "").trim();
  const plateLength = plateNumber.length;
  if (plateLength !== 7 && plateLength !== 8) {
    return plateColor;
  }

  let normalizedColor = plateColor;
  if (plateLength === 8) {
    normalizedColor = "绿牌";
  } else if (yellowPlateVehicleTypes.has(vehicleType || "")) {
    normalizedColor = "黄牌";
  } else {
    normalizedColor = "蓝牌";
  }
...
function normalizeVideoDetectionForDisplay(item: PlateDetection): PlateDetection {
  if (mode.value !== "video") {
    return item;
  }

  const normalizedColor = normalizePlateColorForDisplay(item.plate_number, item.plate_color, item.vehicle_type);

  if (normalizedColor === item.plate_color) {
    return item;
  }

  return {
    ...item,
    plate_color: normalizedColor
  };
}
...
const currentDetections = computed(() => {
  ...
  if (mode.value === "video") {
    if (videoResult.value) {
      return (videoResult.value.detections ?? []).map(normalizeVideoDetectionForDisplay);
    }
    return (videoJob.value?.detections ?? []).map(normalizeVideoDetectionForDisplay);
  }
  return [];
});
...
const displayHistory = computed<HistoryRecordView[]>(() => {
  ...
  return historyRecords.value.map((record) => ({
    ...
    color: normalizePlateColorForDisplay(record.plate_number, record.plate_color, record.vehicle_type),
