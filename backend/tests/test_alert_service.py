...
TEST_DB_PATH = ROOT_DIR / "backend" / "tests" / "test_alert_service.db"
...
class AlertServiceTestCase(unittest.TestCase):
    ...
    def test_behavior_logs_append_new_items(self) -> None:
        service = AlertService(self.db, notifier=StubNotifier())

        first_batch = service.list_behavior_logs(limit=6)
        second_batch = service.list_behavior_logs(limit=6)

        self.assertGreaterEqual(len(first_batch), 6)
        self.assertGreaterEqual(len(second_batch), 6)
        self.assertNotEqual(first_batch[0].id, second_batch[0].id)
        self.assertIn(second_batch[0].source, {"plate-recognition", "police-gesture", "owner-gesture"})
