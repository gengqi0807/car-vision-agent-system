...
def _ensure_police_gesture_record_columns(inspector) -> None:
    ...
    if "user_id" not in columns:
        statements.append("ALTER TABLE police_gesture_records ADD COLUMN user_id INT NULL")
    if "session_id" not in columns:
        statements.append("ALTER TABLE police_gesture_records ADD COLUMN session_id VARCHAR(64) NULL")
