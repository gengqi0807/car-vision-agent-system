...
class PlateService:
    ...
    def recognize_video_bytes(self, video_bytes: bytes, filename: str = "unknown.mp4") -> PlateVideoRecognitionResponse:
        ...
        return result.model_copy(update={"detections": self._normalize_outgoing_video_detections(result.detections)})
    ...
    def get_video_job_status(self, job_id: str) -> PlateVideoJobStatusResponse | None:
        with self._video_jobs_lock:
            ...
            return PlateVideoJobStatusResponse(
                ...
                detections=self._normalize_outgoing_video_detections(job.detections),
    ...
    def _process_video_frame_with_tracking(
        ...
        *,
        ...
        if should_rerecognize:
            ran_recognize = True
            stream_reacquire_mode = state.stream_mode and (
                not state.tracks
                or any(track.misses > 0 for track in state.tracks)
                or any(not track.plate_number for track in state.tracks)
            )
            use_heavy_scan = (
                settings.plate_video_detector_full_frame_fallback and self._should_use_heavy_scan(state)
            ) or (stream_reacquire_mode and self._should_use_heavy_scan(state))
            recognize_started_at = time.perf_counter()
            raw_working_detections = self._recognize_detections(
                working_frame,
                aggressive=stream_reacquire_mode,
    ...
    def _resolve_video_recognition_max_side(self, source_frame, state: PlateProcessingState) -> int:
        if state.stream_mode:
            ...
            if not state.tracks:
                return min(max(configured_limit, 1120), source_longest_side)
            if any(self._is_small_target_track_bbox(track.bbox) for track in active_unread_tracks):
                return min(max(configured_limit, 1280), source_longest_side)
            if active_unread_tracks:
                return min(max(configured_limit, 1152), source_longest_side)
            if any(self._is_large_recognized_track(track) for track in state.tracks):
                return min(configured_limit, 960)
            return min(configured_limit, source_longest_side)
    ...
    def _track_plate(self, gray_frame, track: PlateTrack, *, stream_mode: bool = False) -> bool:
        ...
        stale_frames_since_recognition = max(track.last_seen_frame - track.last_recognized_frame, 0)
        if stream_mode and track.plate_number:
            recognized_bbox = track.last_recognized_bbox or track.bbox
            iou = self._compute_iou(next_bbox, recognized_bbox)
            offset_x_ratio, offset_y_ratio = self._compute_bbox_center_offset_ratio(next_bbox, recognized_bbox)
            if stale_frames_since_recognition >= 1:
                if max_score < 0.74:
                    return False
                if iou < 0.22 and (offset_x_ratio > 0.75 or offset_y_ratio > 0.75):
                    return False
            if stale_frames_since_recognition >= 3:
                if iou < 0.38 or offset_x_ratio > 0.48 or offset_y_ratio > 0.48:
                    return False
        ...
        if stream_mode and track.plate_number and stale_frames_since_recognition > 0:
            # Keep the last detector-confirmed template stable for realtime streams to reduce drift.
        ...
        *,
        ...
        if stream_mode:
            ...
            if stale_frames >= 3:
                ...
                if score < 0.84:
                    return True
                if iou < 0.18 and (offset_x_ratio > 0.85 or offset_y_ratio > 0.85):
                    return True
        if stale_frames <= 8:
            return False
    ...
    def _merge_stream_tracking_and_cached_detections(
        ...
        for detection in cached_detections:
            ...
            best_index = -1
            best_score = 0.0
            for index, tracked in enumerate(merged):
                iou = self._compute_iou(tracked.bbox, detection.bbox)
                offset_x_ratio, offset_y_ratio = self._compute_bbox_center_offset_ratio(tracked.bbox, detection.bbox)
                spatial_score = iou - (offset_x_ratio * 0.08) - (offset_y_ratio * 0.08)
                if iou >= 0.10 or (offset_x_ratio <= 0.85 and offset_y_ratio <= 0.85):
                    if spatial_score > best_score:
                        best_score = spatial_score
                        best_index = index

            if best_index < 0:
                # When a tracking box already exists on the current frame, do not re-introduce
                # a detached cached box from an older frame. That is the main source of "ghost"
                # results sticking around and then attaching to a later vehicle.
                continue

            tracked = merged[best_index]
            if (
                (not (tracked.plate_number or "").strip() or tracked.confidence < detection.confidence)
                and plate_number
            ):
                tracked.plate_number = detection.plate_number
                tracked.plate_color = detection.plate_color
                tracked.vehicle_type = detection.vehicle_type
                tracked.confidence = max(tracked.confidence, detection.confidence)
                if plate_number:
    ...
    def _select_stream_tracking_display_tracks(
        ...
        *,
        ...
        for track in tracks:
            ...
            offset_x_ratio, offset_y_ratio = self._compute_bbox_center_offset_ratio(track.bbox, recognized_bbox)
            if (
                track.last_tracking_score >= 0.76
                and (
                    iou >= 0.16
                    or (offset_x_ratio <= 0.82 and offset_y_ratio <= 0.82)
                )
            ):
                selected.append(track)
        return selected
    ...
    def _should_display_stream_cached_detections(
        ...
        *,
        ...
        max_version_gap = max(
            ...
            int(
                round(
                    max(settings.plate_stream_max_fps, 1)
                    * min(max(settings.plate_stream_detection_hold_seconds, 0.18), 0.28)
                )
            ),
        )
        if version_gap > max_version_gap:
            return False
        max_age = max(submit_interval * 2.2, min(max(settings.plate_stream_detection_hold_seconds, 0.18), 0.30))
    ...
    def _pick_stable_track_color(
        ...
        *,
        ...
        best_color = self._normalize_plate_color_for_plate(
            max(color_votes.items(), key=lambda item: item[1])[0],
            plate_number,
        )
        ...
        fallback_color = self._normalize_plate_color_for_plate(fallback_color, plate_number)
        if blue_votes <= 0 and yellow_votes <= 0:
            if fallback_color in {PLATE_COLOR_BLUE, PLATE_COLOR_YELLOW}:
                return fallback_color
            if best_color == PLATE_COLOR_GREEN:
                return PLATE_COLOR_BLUE
            return best_color
    ...
    def _finalize_video_detections(self, best_detections: dict[str, VideoDetectionStats]) -> list[PlateDetection]:
        ...
        accepted = [
            self._stabilize_video_detection_color(
                item.model_copy(
                    update={
                        "plate_color": self._normalize_plate_color_for_plate(item.plate_color, item.plate_number)
                    }
        ...
        accepted = [
            self._force_final_video_plate_color(
                item.model_copy(
                    update={
                        "vehicle_type": self._fallback_vehicle_type_for_bbox(
                            item.vehicle_type,
    ...
    def _normalize_outgoing_video_detections(self, detections: list[PlateDetection]) -> list[PlateDetection]:
        normalized: list[PlateDetection] = []
        for item in detections:
            detection = item.model_copy(deep=True) if hasattr(item, "model_copy") else item
            ...
            detection = self._force_final_video_plate_color(detection)
            normalized.append(detection)
        return normalized

    def _force_final_video_plate_color(self, detection: PlateDetection) -> PlateDetection:
        plate_number = (detection.plate_number or "").strip()
        plate_length = len(plate_number)
        if plate_length == 8:
            forced_color = PLATE_COLOR_GREEN
        elif plate_length == 7:
            normalized_vehicle_type = self._normalize_vehicle_type_from_label(detection.vehicle_type)
            if normalized_vehicle_type in {VEHICLE_TYPE_TRUCK, VEHICLE_TYPE_BUS, VEHICLE_TYPE_CRANE}:
                forced_color = PLATE_COLOR_YELLOW
            else:
                forced_color = PLATE_COLOR_BLUE
        else:
            return detection

        if detection.plate_color == forced_color:
            return detection
        return detection.model_copy(update={"plate_color": forced_color})
    ...
    def _stabilize_video_detection_color(self, detection: PlateDetection) -> PlateDetection:
        ...
        normalized_vehicle_type = self._normalize_vehicle_type_from_label(detection.vehicle_type)
        if detection.plate_color == PLATE_COLOR_GREEN:
            return PlateDetection(
                plate_number=detection.plate_number,
                plate_color=PLATE_COLOR_BLUE
                if normalized_vehicle_type not in {VEHICLE_TYPE_TRUCK, VEHICLE_TYPE_BUS, VEHICLE_TYPE_CRANE}
                else PLATE_COLOR_YELLOW,
                vehicle_type=detection.vehicle_type,
                confidence=detection.confidence,
                bbox=list(detection.bbox),
            )
        ...
        if normalized_vehicle_type in {VEHICLE_TYPE_CAR, VEHICLE_TYPE_JEEP, VEHICLE_TYPE_UNKNOWN}:
            return PlateDetection(
                plate_number=detection.plate_number,
                plate_color=PLATE_COLOR_BLUE,
                vehicle_type=detection.vehicle_type,
    ...
    def _tracking_match_threshold_for_mode(self, *, stream_mode: bool) -> float:
        if stream_mode:
            return max(float(settings.plate_stream_tracking_match_threshold), 0.0)
    ...
    def _tracking_template_update_alpha_for_mode(self, *, stream_mode: bool) -> float:
        if stream_mode:
            return max(0.0, min(float(settings.plate_stream_tracking_template_update_alpha), 1.0))
