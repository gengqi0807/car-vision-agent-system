...
POSE_MODEL_PATH = _resolve_project_path(POSE_MODEL_PATH)
HAND_MODEL_PATH = os.environ.get(
    "POLICE_HAND_MODEL",
    os.path.join("backend", "hand_landmarker.task")
)
HAND_MODEL_PATH = _resolve_project_path(HAND_MODEL_PATH)
...
# --- 多人检测场景下，用于锁定追踪目标人物的参数 ---
# 多人物检测的最大人数（仅摄像头模式下生效，可通过 --num-poses 覆盖）
NUM_POSES_MULTI = 5
# 目标人物锁定：预热后前 N 帧内锁定画面中最大/最中心的人物
TARGET_LOCK_FRAMES = 10
# 目标人物位置匹配阈值（归一化坐标），用于帧间追踪匹配
TARGET_TRACK_THRESH = 0.35
